    ...
    public function ejecutarLogin($usr, $pass)
    {
        ...
        switch ($res)     
        {
            ...
            case CodRes::NO_CONNECTED:
                ...
                break;
            case CodRes::ACCOUNT_LOCKED:
                header('Location: ../index.php?msg=act_lock');
                break;
            case CodRes::EXITO:
...
public function ejecutarProcesarClienteNaturalM($cedC,$emailC, $telfC, $telcC)
    ...
    public function ejecutarMostrarFormularioVendedorM($idVendedor)
    {
     ...
     public function ejecutarMostrarReservaModulos()
     {
         //$this->genHtml->imprimirReservarModulos();
         $this->ejecutarMostrarCrearReserva();
     }
     
     public function ejecutarMostrarVistaBusquedaReserva()
     {
         $res=$this->control->srvObtenerVistaReservas($ids, $tipos_serv,$proveedores, $ciudades, 
                                              $costos, $descripciones,$fecha,$hora, $numP);
         switch ($res)
         {
            case CodRes::SIN_REGISTROS:
                $this->genHtml->imprimirVistaBusquedaReserva($ids, $tipos_serv,$proveedores, $ciudades, 
                                              $costos, $descripciones,$fecha,$hora, 0);
                break;
            case CodRes::EXITO:
                $this->genHtml->imprimirVistaBusquedaReserva($ids, $tipos_serv,$proveedores, $ciudades, 
                                              $costos, $descripciones,$fecha,$hora, $numP);
    ...
    public function ejecutarMostrarCrearReserva()
    {
        $res = CodRes::SIN_REGISTROS;
        $res1=$this->control->srvObtenerCiudades($idsC,$nomC, $cantC);
        $res2=$this->control->srvObtenerTiposServicios($idsTS,$nomTS, $cantTS);
        $res3=$this->control->srvObtenerProveedores($idsP,$nomP, $cantP);
        if ($res1 == CodRes::SIN_REGISTROS || $res2 == CodRes::SIN_REGISTROS ||
                $res3 == CodRes::SIN_REGISTROS)
        {
                $info = "SIN REGISTROS";
                $msg = "NO ESTAN COMPLETOS LOS DATOS BÁSICOS!";
                $this->genHtml->imprimirResultado($info, $msg);
        }
        else
        {
           $this->genHtml->imprimirMostrarFormularioReserva($idsC,$nomC, $cantC,
                                                                 $idsTS,$nomTS, $cantTS,
                                                                 $idsP, $nomP, $cantP);
        }
    }
    ...
    public function ejecutarMostrarModuloPropaganda()
    {
        $this->genHtml->imprimirModuloPropaganda("HOME");
    }    
    
    public function ejecutarMostrarModuloMenuLateralArrancar()
    {
        $this->genHtml->imprimirModuloMenuLateralArrancar();
    }
    
    public function ejecutarMostrarBusquedaReserva()
    {
        $res=$this->control->srvObtenerVistaReserva($ids_producto, $datos_producto, $desc_producto, $costo_producto, $num);
        switch ($res)
        {
            case CodRes::SIN_REGISTROS:
                $info = "SIN REGISTROS";
                ...
                break;
            case CodRes::EXITO:
                $this->genHtml->imprimirVistaBusquedaReserva($ids_producto, $datos_producto, $desc_producto, $costo_producto, $num);
                break;
            default:
                $info = "ERORR DESCONOCIDO";
                $msg = "FAVOR CONSULTE AL ADMINISTRADOR DEL SISTEMA!";
    ...
    public function ejecutarMostrarCarroCompras($data, $num)
    {
        if($num==0)
            $this->genHtml->imprimirCarritoCompras($data, 0);
        else
            $this->genHtml->imprimirCarritoCompras($data, $num);
    }
    
    public function ejecutarMostrarFiltroServicios0($tipo, $proveedor, $ciudad, $fecha)
    {
        $num=0;
        if($tipo!=-1 && $proveedor!=-1 && $ciudad!=-1)
            $res = $this->control->srvObtenerFiltroServicios6($ids, $tipos_serv,$proveedores, $ciudades, 
                                                          $costos, $descripciones,$fecha,$hora,
                                                          $tipo, $proveedor, $ciudad, $fecha, $num);
        else if($tipo!=-1 && $proveedor!=-1 && $ciudad==-1)
            $res = $this->control->srvObtenerFiltroServicios5($ids, $tipos_serv,$proveedores, $ciudades, 
                                                          $costos, $descripciones,$fecha,$hora,
                                                          $tipo, $proveedor, $ciudad, $fecha, $num);
        else if($tipo!=-1 && $proveedor==-1 && $ciudad!=-1)
            $res = $this->control->srvObtenerFiltroServicios4($ids, $tipos_serv,$proveedores, $ciudades, 
                                                          $costos, $descripciones,$fecha,$hora,
                                                          $tipo, $proveedor, $ciudad, $fecha, $num);
        else if($tipo!=-1 && $proveedor==-1 && $ciudad==-1)
            $res = $this->control->srvObtenerFiltroServicios3($ids, $tipos_serv,$proveedores, $ciudades, 
                                                          $costos, $descripciones,$fecha,$hora,
                                                          $tipo, $proveedor, $ciudad, $fecha, $num);
        //-100
        else if($tipo==-1 && $proveedor!=-1 && $ciudad!=-1)
            $res = $this->control->srvObtenerFiltroServicios2($ids, $tipos_serv,$proveedores, $ciudades, 
                                                          $costos, $descripciones,$fecha,$hora,
                                                          $tipo, $proveedor, $ciudad, $fecha, $num);
        //-10-1
        else if($tipo==-1 && $proveedor!=-1 && $ciudad==-1)
            $res = $this->control->srvObtenerFiltroServicios1($ids, $tipos_serv,$proveedores, $ciudades, 
                                                          $costos, $descripciones,$fecha,$hora,
                                                          $tipo, $proveedor, $ciudad, $fecha, $num);
        //-1-10
        else if($tipo==-1 && $proveedor==-1 && $ciudad!=-1)
            $res = $this->control->srvObtenerFiltroServicios0($ids, $tipos_serv,$proveedores, $ciudades, 
                                                          $costos, $descripciones,$fecha,$hora,
                                                          $tipo, $proveedor, $ciudad, $fecha, $num);
        else if($tipo==-1 && $proveedor==-1 && $ciudad==-1)
            $res = $this->control->srvObtenerFiltroServicios7($ids, $tipos_serv,$proveedores, $ciudades, 
                                                          $costos, $descripciones,$fecha,$hora,
                                                          $tipo, $proveedor, $ciudad, $fecha, $num);
        switch ($res)
        {
            ...
            case CodRes::EXITO:
                $this->genHtml->imprimirVistaBusquedaReserva($ids, $tipos_serv,$proveedores, $ciudades, 
                                              $costos, $descripciones,$fecha,$hora,$num);
                break;
            default:
                $info = "ERORR DESCONOCIDO";
                $msg = "FAVOR CONSULTE AL ADMINISTRADOR DEL SISTEMA!";
    ...
    public function ejecutarProcesarReservar($data, $numData)
    {
        $ref = -1;
        $num = -1;
        if(!$this->control->srvObtenerSecuenciaReserva($ref, $num))
        {            
            $info = "ERORR OBTENIENDO DATOS".$ref;
            $msg = "FAVOR CONSULTE AL ADMINISTRADOR DEL SISTEMA!";
            $this->genHtml->imprimirResultado($info, $msg);
            return;
        }
        $numCols=0;
        $res = $this->control->srvProcesarReservar($ref, $data, $numData, $numCols);
        ...
        switch ($res)
        {
            case CodRes::NO_CONNECTED:
                ...
                break;
            case CodRes::EXITO:
                $info = "SERVICIOS RESERVADOS";
                $msg = "FAVOR GUARDE SU NUMERO DE RESERVA:-->".$ref."<--\n<br>".
                       "Lo Necesitara para confirmar su reserva. \n<br>".
                       "Recuerde confirmar su reserva 12 horas antes del 'Evento Reservado' contrario esta será cancelada.\n<br>".
                       "Cantidad de servicios reservados: ".$numCols." \n<br>";
                $this->genHtml->imprimirResultado($info, $msg);
                break;
            default:
                $info = "ERORR DESCONOCIDO";
    ...
    public function ejecutarMostrarBuscarReserva()
    {
        $this->genHtml->imprimirMostrasBuscarReserva();
    }
    
    public function ejecutarMostrarCarroComprasS($data, $num)
    {
        if($num==0)
            $this->genHtml->imprimirCarritoComprasS($data, 0);
        else
            $this->genHtml->imprimirCarritoComprasS($data, $num);
    }
    
    public function ejecutarBuscarReservas($ref)
    {
        $res = $this->control->srvBuscarReservas($ref, $data, $num);
        ...
        switch ($res)
        {
            ...
            case CodRes::SIN_REGISTROS:
                ...
                $msg = "FAVOR VALIDE LA INFORMACION INGRESADA\n<BR>".
                        "Y recuerde que si ud. no confirmo su reserva con las 12 hrs de anticipación\n<br>".
                        "esta fue cancelada por el sistema de forma automática";
                $this->genHtml->imprimirResultado($info, $msg);
                break;
            case CodRes::EXITO:
                $this->ejecutarMostrarCarroComprasS($data, $num);
                break;
            default:
                $info = "ERORR DESCONOCIDO";
                $msg = "FAVOR CONSULTE AL ADMINISTRADOR DEL SISTEMA!";
    ...
    public function ejecutarConfirmarReservas($data, $num)
    {
        $res=$this->control->srvConfirmarReservas($data, $num, $numrows);
        ...
        switch ($res)
        {
            ...
            case CodRes::SIN_REGISTROS:
                ...
                break;
            case CodRes::EXITO:
                $info = "RESERVAS CONFIRMADAS";
                $msg = "SUS RESERVAS FUERON CONFIRMADAS \n<br>".
                       "Recuerde que debe realizar el pago de estos servicios. \n<br>".
                       "Cantidad de reservas Confirmadas: ".$numrows." \n<br>";
                $this->genHtml->imprimirResultado($info, $msg);
                break;
            default:
                $info = "ERORR DESCONOCIDO";
    ...
    public function ejecutarBorrarReservas($data, $num)
    {
        $res=$this->control->srvBorrarReservas($data, $num, $numrows);
        ...
        switch ($res)
        {
            ...
            case CodRes::SIN_REGISTROS:
                ...
                break;
            case CodRes::EXITO:
                $info = "RESERVAS CANCELADAS";
                $msg = "SUS RESERVAS FUERON CANCELADAS \n<br>".
                       "Cantidad de reservas Eliminadas: ".$numrows." \n<br>";
                $this->genHtml->imprimirResultado($info, $msg);
                break;
            default:
                $info = "ERORR DESCONOCIDO";
