<?php
    ...
    if(isset($_POST['form_type']))
    {
        ...
        switch ($ft)
        {
            case "form_confirmar_reservas":
                if(isset ($_POST['dataC_id']) && isset ($_POST['tarea_id']) && $_POST['tarea_id']=="C")
                    $vista->ejecutarConfirmarReservas($_POST['dataC_id'], $_POST['total_servicios']);
                else if(isset ($_POST['tarea_id']) && isset ($_POST['dataD_id']) && $_POST['tarea_id']=="D")
                    $vista->ejecutarBorrarReservas($_POST['dataD_id'], $_POST['total_servicios']);
                break;
            case "buscar_reserva":
                if(isset ($_POST['codigo_reserva']))
                        $vista->ejecutarBuscarReservas($_POST['codigo_reserva']);
                break;
            case "form_reservar_servicios":
                if (isset ($_POST['dataR_id']) || isset ($_POST['total_servicios']))
                    $vista->ejecutarProcesarReservar($_POST['dataR_id'], $_POST['total_servicios']);
                else
                    $vista->ejecutarmostrarError($ft);
                break;
            case "filtrar_reserva":
                if (isset ($_POST['id_tipoS'])){
                    if(isset ($_POST['id_ciudad'])){
                        if(isset ($_POST['id_proveedor'])) {
                                $vista->ejecutarMostrarFiltroServicios0($_POST['id_tipoS'],
                                                                        $_POST['id_proveedor'],
                                                                        $_POST['id_ciudad'],
                                                                        $_POST['fecha_reserva']);
                        }
            ...
            case "adicionar_carrito":
                if(isset ($_POST['data_id']) && isset ($_POST['total_items']))
                    $vista->ejecutarMostrarCarroCompras($_POST['data_id'],$_POST['total_items']);
                else
                    $vista->ejecutarmostrarError($ft);
                break;
            case "tipo_documento":
